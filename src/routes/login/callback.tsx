...
import styles from '../../styles/login/callback.module.css';
...
const Callback: Component = (props) => {
	...
	if (hd !== 'chatham-nj.org') {
		window.location.href =
			'https://xfrh1g.sse.codesandbox.io/login?error=SCHOOL_ACCOUNT';
	}
	if (state !== window.localStorage.getItem('googleAuthState')) {
		window.location.href =
			'https://xfrh1g.sse.codesandbox.io/login?error=INVALID_SESSION';
	}
	return (
